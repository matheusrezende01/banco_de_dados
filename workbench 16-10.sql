...
-- 1
select clientes.nome, pedidos.id from pedidos
 ...
 -- 2
select  count(*) as 'total de pedidos' from pedidos;
 -- 3
 ...
 where clientes.nome = 'Alexandre Santod';
-- 5
select nome, telefone, pedidos.id as pedido, data, valor from clientes
right join pedidos on clientes.id = pedidos.cliente_id;
-- 6
select sum(valor) as valor_total_pedidos from pedidos;
-- 7
...
-- 10
select cliente_id, clientes.nome, sum(pedidos.valor) as valor_total, count(pedidos.id) as total_pedidos from pedidos
...
-- 13
select nome, count(pedidos.id) as quantidade_de_pedidos from pedidos
...
limit 1;
-- 20 
select nome, pedidos.id as pedidos,data, valor from clientes
left join pedidos on pedidos.cliente_id = clientes.id;
-- 21
select nome,  sum(valor) as valor_total from pedidos
